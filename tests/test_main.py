...
class TestMain:
    ...
    def test_register_wrong_credentials(self, mock_validate_credentials, client):
    ...
    def test_register_user_already_registered(self, mock_validate_credentials_user, mock_add_user, client):
        response = client.post("/register", json={"user": "user", "passwd": "password"})
    ...
    def test_register_ok(self, mock_validate_credentials_user, mock_add_user, mock_get_hash, mock_add_user_storage, client):
        response = client.post("/register", json={"user": "user", "passwd": "password"})
        assert response.status_code == HTTPStatus.OK
        assert response.data == b'{"hash": "6a8d8ae8-5fc1-4df0-93db-1ecb7a9af0aa"}'

    @patch.object(main, "validate_credentials", return_value=False)
    def test_login_wrong_credentials(self, mock_validate_credentials, client):
    ...
    def test_login_not_valid_user(self, mock_validate_credentials_user, mock_is_user_valid, client):
        response = client.post("/login", json={"user": "user", "passwd": "password"})
    ...
    @patch.object(main, "get_hash", return_value='6a8d8ae8-5fc1-4df0-93db-1ecb7a9af0aa')
    @patch.object(main, "add_user_storage", return_value=None)
    def test_login_ok(self, mock_validate_credentials_user, mock_is_user_valid,  mock_get_hash, mock_add_user_storage, client):
        response = client.post("/login", json={"user": "user", "passwd": "password"})
        assert response.status_code == HTTPStatus.OK
        assert response.data == b'{"hash": "6a8d8ae8-5fc1-4df0-93db-1ecb7a9af0aa"}'

    @patch.object(main, "get_uuid_if_valid", return_value=False)
    def test_post_cpu_wrong_hash(self, mock_get_uuid_if_valid, client):
    ...
    def test_post_cpu_unauthorized(self, mock_get_uuid_if_valid, mock_post_data, client):
        response = client.post("/cpu", json={"hash": "6a8d8ae8-5fc1-4df0-93db-1ecb7a9af0aa", "cpu": 20.0, "time": 1648481187.7068})
        assert response.status_code == HTTPStatus.UNAUTHORIZED
    ...
    def test_post_cpu_ok(self, mock_get_uuid_if_valid, mock_post_data, client):
        response = client.post("/cpu", json={"hash": "6a8d8ae8-5fc1-4df0-93db-1ecb7a9af0aa", "cpu": 20.0, "time": 1648481187.7068})
        assert response.status_code == HTTPStatus.OK
        assert response.data == (b'{"payload": {"cpu": 20.0, "hash": "6a8d8ae8-5fc1-4df0-93db-1ecb7a9af0aa", "time": 1648481187.7068}}')

    @patch.object(main, "get_uuid_if_valid", return_value=False)
    def test_get_cpu_wrong_hash(self, mock_get_uuid_if_valid, client):
    ...
    def test_get_cpu_unauthorized(self, mock_get_uuid_if_valid, mock_get_data, client):
        time_value = time.time()
        response = client.get("/cpu", json={"hash": "6a8d8ae8-5fc1-4df0-93db-1ecb7a9af0aa", "time": time_value})
        assert response.status_code == HTTPStatus.UNAUTHORIZED
    ...
    def test_get_cpu_ok(self, mock_get_uuid_if_valid, mock_post_data, client):
