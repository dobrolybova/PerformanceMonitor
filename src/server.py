...
def login() -> Response:
    ...
    u = userHandler.add_user(user, passwd)
    if not u:
        return resp(HTTPStatus.UNAUTHORIZED, {"reason": "Wrong/no user or password, please provide correct"})
    user_hash = userHandler.get_hash(user)
    storageHandler.add_user_storage(str(user_hash))
...
def post_cpu() -> Response:
    ...
    if storageHandler.post_data(str(user_hash), request.json):
...
def get_cpu() -> Response:
    ...
    if not user_hash:
        return resp(HTTPStatus.BAD_REQUEST, {"reason": "Bad arguments"})
    data = storageHandler.get_data(str(user_hash), timerange)
    if not data:
